...
plt.figure(figsize=(15, 15))  # Aumentar altura para más espacio vertical
...
# Añadir puntos (sin cambios)
sns.stripplot(x='NLQ_ID', y='Tiempo', data=melted, 
             color='#2C3E50', size=6, alpha=0.4, 
             jitter=0.25)

# Personalización avanzada: escala symlog
ax.set_yscale('symlog', linthresh=1, linscale=1)  # Escala lineal hasta 1s, luego logarítmica
y_ticks = [0.1, 0.2, 0.5, 1, 2, 5, 10]
ax.set_yticks(y_ticks)
ax.set_yticklabels([f"{t} s" for t in y_ticks], fontsize=20, fontweight='semibold')

# Eje X y otros elementos (sin cambios)
nlq_labels = [f"NLQ {int(x)}" for x in sorted(melted['NLQ_ID'].unique())]
...
plt.savefig('Times_per_nlqys.pdf', dpi=300, bbox_inches='tight')
plt.close()